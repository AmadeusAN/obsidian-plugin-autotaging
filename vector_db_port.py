...
def assign_tags(
    node: dict, doc_contents: list[str], index2ids: dict, threshold: float = 0.5
) -> dict:
    """
    自底向上递归生成tag树。
    :param node: 当前节点
    ...
    if norm_dist < threshold:
        ...
        parent_tag = llm_generate_tag(
            f"基于这些子tag或主题，生成一个概括性的父tag（保持简短，如'Category/Sub'）: {child_tags_str}"
        )
...
def generate_tags_for_alldoc(data_metals):
